...
@Database(entities = [User::class, LibraryFile::class, Checklist::class, ChecklistItem::class, ReadingSession::class, Annotation::class, Bookshelf::class, BookshelfItem::class], version = 10, exportSchema = false)
abstract class InfeReadDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE library_files ADD COLUMN finishedAt INTEGER NOT NULL DEFAULT 0")
            }
        }

        val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_library_files_format` ON `library_files` (`format`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_library_files_rating` ON `library_files` (`rating`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_library_files_addedAt` ON `library_files` (`addedAt`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_library_files_isBookmarked` ON `library_files` (`isBookmarked`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_library_files_isToRead` ON `library_files` (`isToRead`)")
            }
        }
