...
object BookDownloader {
    ...
    suspend fun downloadBook(context: Context, downloadUrl: String, fileName: String): File? {
        return withContext(Dispatchers.IO) {
            try {
                val isOffline = context.getSharedPreferences("settings_prefs", Context.MODE_PRIVATE).getBoolean("is_offline_mode", false)
                if (isOffline) {
                    Log.e("BookDownloader", "Offline Mode Enabled: Download aborted for $downloadUrl")
